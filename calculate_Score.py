...
sparsity= float(input("Insert prune ratio: "))
...
def count_conv2d(m, x, y):
    ...
    #Nice Formatting
    print("{:<10}: S_c={:<4}, F_in={:<4}, F_out={:<4}, P={:<5}, params={:<10}, operations={:<20}".format("Conv2d",sh,fin,fout,x.size()[2:].numel(),total_params,int(total_ops)))
...
def count_linear(m, x, y):
    ...
    total_ops = total_ops*(1-sparsity)
    total_params = int(m.total_params.item()*(1-sparsity))
    print("Linear: F_in={}, F_out={}, params={}, operations={}".format(m.in_features,m.out_features, total_params,int(total_ops)))
    m.total_ops += torch.Tensor([int(total_ops)])
...
def main():
    # PreActResnet18 - [1111] calculate using thop library
    #ref_params = 4946592
    #ref_flops  = 254691328
    ...
    ref_params = 11217316
    ref_flops  = 557664256
    loaded_cpt = torch.load(f'ckpts/tests/{ckpt_file}')
