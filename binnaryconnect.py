...
class BC():
    ...
    def binarization(self):
        ...
        #Determinist binarisation
        for index in range(self.num_of_params):
            #self.target_modules[index] = list(map(lambda row: list(map(lambda x: -1 if x<0 else 1, row)), self.saved_params[index])) 
            binary_params = torch.where(self.saved_params[index] < 0, torch.tensor(-1), torch.tensor(1))
            self.target_modules[index] = binary_params 
    ...
    def clip(self):
        ...
        for index in range(self.num_of_params):
            #self.target_modules[index] = list(map(lambda row: list(map(nn.Hardtanh(), row)), self.saved_params[index])))
            m = nn.Hardtanh(-1,1)
            self.target_modules[index] = m(self.saved_params[index])
    def forward(self,x):
